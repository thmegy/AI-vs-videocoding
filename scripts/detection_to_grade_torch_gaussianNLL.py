...
def test_loop(dataloader, model, loss_fn):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            mu, sigma = model(X)
            test_loss += loss_fn(mu, y.squeeze(), sigma).item()
            pred_list += (mu.squeeze()).tolist()
...
#    x_train, x_test, y_train, y_test = train_test_split(df.iloc[:,2:].values, df['NOTE'].values, test_size=0.2, random_state=42)
    x_train, x_test, y_train, y_test = train_test_split(df['Moy_degradation'].values[:,None], df['NOTE'].values, test_size=0.2, random_state=42) # for fake data
...
#    train_loss_fn = nn.MSELoss(reduction='none')
    ...
    test_loss_fn = nn.GaussianNLLLoss() # get loss for each individual element
    ...
    ax4.errorbar(target_list, pred_list, yerr=sigma_list, fmt='o')
